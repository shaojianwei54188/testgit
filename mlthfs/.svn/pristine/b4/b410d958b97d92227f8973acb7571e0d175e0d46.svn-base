layui.config({
    base: './static/common/js/user/'
}).extend({
    ajaxExtention: 'ajaxExtention',//加载自定义扩展，每个业务js都需要加载这个ajax扩展
    $tool: 'tool'
}).use(['form', 'layer', 'jquery', 'table', 'laypage','laytpl', 'ajaxExtention', '$tool'], function () {
    var form = layui.form,
    ...
    function delUser(id){
        layer.confirm('确认删除吗？', function (confirmIndex) {
            ...
            //向服务端发送删除指令
            url = $tool.getContext() + '/sysUser/delete.do';
            var req = {
                id: id
            };
    ...
    function initPwd(id){
        layer.confirm('确认初始化密码吗？', function (confirmIndex) {
            layer.close(confirmIndex);//关闭confirm
            //向服务端发送删除指令
            url = $tool.getContext() + '/sysUser/initPwd.do';
            var req = {
                id: id
            };
